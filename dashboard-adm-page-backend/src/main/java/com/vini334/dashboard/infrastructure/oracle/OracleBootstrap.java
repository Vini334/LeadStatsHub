...
public class OracleBootstrap {
  ...
  public void init() {
    ...
    String fnDailyEngagement = """
      ...
      RETURN NUMBER IS
        v_interactions NUMBER := 0; v_active_users NUMBER := 0;
      BEGIN
        SELECT COUNT(*) INTO v_interactions FROM INTERACTIONS WHERE TRUNC(CREATED_AT)=TRUNC(p_day);
    ...
    String fnAvgEngPerUser = """
      ...
      RETURN NUMBER IS
        v_avg NUMBER;
      BEGIN
        SELECT AVG(cnt) INTO v_avg FROM (SELECT COUNT(*) cnt FROM INTERACTIONS GROUP BY USER_ID);
        RETURN NVL(v_avg,0);
      EXCEPTION WHEN NO_DATA_FOUND THEN RETURN 0; WHEN OTHERS THEN RETURN 0;
      END;
      """;
    ...
    String fnUserSummaryJson = """
      ...
      BEGIN
        ...
        v_json_clob := TO_CLOB(v_json_vc2); RETURN v_json_clob;
      EXCEPTION
    ...
    String procUserReport = """
      CREATE OR REPLACE PROCEDURE PROC_USER_CONSUMPTION_REPORT(
        ...
        v_posts NUMBER:=0; v_interactions NUMBER:=0; v_likes NUMBER:=0; v_errmsg VARCHAR2(2000);
      BEGIN
        ...
        SELECT COUNT(*) INTO v_interactions FROM INTERACTIONS
         WHERE USER_ID=p_user_id AND CREATED_AT>=p_from AND CREATED_AT<p_to;

        SELECT COUNT(*) INTO v_likes FROM INTERACTIONS
         WHERE USER_ID=p_user_id AND UPPER(TYPE)='LIKE'
